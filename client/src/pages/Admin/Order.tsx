...
const OrderPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await get<OrderAdmin>(
        ENDPOINTS.LIST_ORDER + `?page=${page}`,
        token
      );
  ...
  const handleUpdatePayment = async (orderId: string) => {
    try {
      await put(ENDPOINTS.UPDATE_PAYMENT_STATUS + `/${orderId}`, {}, token);
      toast({
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <TableCell className="text-muted-foreground">
                  {new Date(
                    String(order.orderDate).replace("ICT", "+0700")
